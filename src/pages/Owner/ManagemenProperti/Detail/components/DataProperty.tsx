...
const DaftarProperti = (props: { dataState: any; setDataState: any; listRules: any; indonesia: any }) => {
  ...
  useEffect(() => {
    try {
      if (props.dataState.latitude && props.dataState?.longitude) {
        setPosition({ lat: props.dataState.latitude, lng: props.dataState?.longitude });
        props.setDataState((prev: any) => ({ ...prev, latitude: props.dataState.latitude, longitude: props.dataState?.longitude }));
      }
    } catch (error) {}
  }, [props.dataState?.latitude, props.dataState?.longitude]);
  ...
  return (
    <Stack gap={"25px"}>
      {/* <pre>
        {JSON.stringify(
          {
            provinsi: props.dataState.provinsi,
            kota: props.dataState.kota,
            kecamatan: props.dataState.kecamatan,
          },
          null,
          2
        )}
      ...
      {(props.dataState?.provinsi || null) && props.indonesia.length > 0 ? (
        <Stack>
          ...
          <Select
            ...
            border={customBorder()}
            color={inputColor()}
            value={props.dataState?.kota || null}
            onChange={(e) => {
              props.setDataState((prev: any) => ({ ...prev, kecamatan: "" }));
      ...
      {(props.dataState?.kota || null) && props.indonesia.length > 0 ? (
        <Stack>
          ...
          <Select
            ...
            border={customBorder()}
            color={inputColor()}
            value={props.dataState?.kecamatan || null}
            onChange={(e) => {
